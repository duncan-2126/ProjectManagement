...
// CreateRelationship creates a new relationship between TODOs
func (db *DB) CreateRelationship(sourceID, targetID, relType string) (*Relationship, error) {
	rel := Relationship{
		ID:        uuid.New().String(),
		SourceID:  sourceID,
		TargetID:  targetID,
...
// GetRelationshipsForTODO returns all relationships for a TODO
func (db *DB) GetRelationshipsForTODO(todoID string) ([]Relationship, error) {
	var relationships []Relationship
...
// GetDependencies returns what a TODO depends on
func (db *DB) GetDependencies(todoID string) ([]Relationship, error) {
	var relationships []Relationship
	if err := db.Where("source_id = ? AND type = ?", todoID, "depends_on").Find(&relationships).Error; err != nil {
		return nil, err
	}
...
// GetBlockers returns what blocks a TODO
func (db *DB) GetBlockers(todoID string) ([]Relationship, error) {
	var relationships []Relationship
	if err := db.Where("target_id = ? AND type = ?", todoID, "depends_on").Find(&relationships).Error; err != nil {
		return nil, err
	}
	return relationships, nil
}
...
// GetChildren returns subtasks of a TODO
func (db *DB) GetChildren(todoID string) ([]Relationship, error) {
	var relationships []Relationship
	if err := db.Where("source_id = ? AND type = ?", todoID, "parent").Find(&relationships).Error; err != nil {
		return nil, err
	}
...
// GetParent returns the parent of a TODO
func (db *DB) GetParent(todoID string) (*Relationship, error) {
	var rel Relationship
	if err := db.Where("target_id = ? AND type = ?", todoID, "parent").First(&rel).Error; err != nil {
		return nil, err
	}
	return &rel, nil
}
...
// RemoveAllRelationshipsForTODO removes all relationships for a TODO
func (db *DB) RemoveAllRelationshipsForTODO(todoID string) error {
	return db.Where("source_id = ? OR target_id = ?", todoID, todoID).Delete(&Relationship{}).Error
}

// CheckCircularDependency checks if adding a dependency would create a cycle
func (db *DB) CheckCircularDependency(sourceID, targetID string) (bool, error) {
	// DFS to detect cycle
	visited := make(map[string]bool)
	var dfs func(current string, target string) bool
	dfs = func(current string, target string) bool {
		if current == target {
			return true
		}
		if visited[current] {
		...
		var deps []Relationship
		db.Where("source_id = ? AND type = ?", current, "depends_on").Find(&deps)
		for _, dep := range deps {
			if dfs(dep.TargetID, target) {
				return true
			}
		}
		return false
	...
	return dfs(targetID, sourceID), nil
}
...
func (db *DB) ValidateRelationships() (map[string][]string, error) {
	...
	// Check for broken links (references to non-existent TODOs)
	var allRelationships []Relationship
	db.Find(&allRelationships)

	for _, rel := range allRelationships {
		// Check if source exists
		var source TODO
		if err := db.First(&source, "id = ?", rel.SourceID).Error; err != nil {
			issues["broken_links"] = append(issues["broken_links"], fmt.Sprintf("Relationship %s references non-existent source: %s", rel.ID, rel.SourceID))
		}
		// Check if target exists
		var target TODO
		if err := db.First(&target, "id = ?", rel.TargetID).Error; err != nil {
			issues["broken_links"] = append(issues["broken_links"], fmt.Sprintf("Relationship %s references non-existent target: %s", rel.ID, rel.TargetID))
		}
	}
	...
	for _, rel := range allRelationships {
		if rel.Type == "depends_on" {
			hasCycle, err := db.CheckCircularDependency(rel.SourceID, rel.TargetID)
			if err != nil {
				return nil, err
			}
			if hasCycle {
				issues["circular_dependencies"] = append(issues["circular_dependencies"], fmt.Sprintf("Circular dependency: %s depends on %s", rel.SourceID, rel.TargetID))
			}
		}
	}
